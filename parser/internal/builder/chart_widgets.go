...
func generateBarChart(sourceDir, filename, title string) string {
	...
	return generateBarChartFromJSON(content, title)
}

func generateBarChartFromJSON(content []byte, title string) string {
	...
	for _, d := range data {
		heightPx := (d.Value / max) * 180.0
		html += fmt.Sprintf(`
		<div class="tamarind-barchart-col" style="flex: 1; display:flex; flex-direction:column; justify-content: flex-end; align-items:center; min-width: 40px;">
			<div class="tamarind-barchart-val" style="font-size: 0.8rem; margin-bottom: 5px; color: currentColor;">%.1f</div>
			<div class="tamarind-barchart-bar" style="width: 100%%; max-width: 60px; height: %.1fpx; background-color: var(--chart-1, var(--primary-color, #3b82f6)); border-radius: 4px 4px 0 0; transition: height 0.5s ease-out;"></div>
			<div class="tamarind-barchart-lbl" style="font-size: 0.8rem; margin-top: 8px; text-align:center; color: currentColor;">%s</div>
		</div>`, d.Value, heightPx, d.Label)
	}
	html += `</div></div>`
	return html
}

func generatePieChart(sourceDir, filename, title string) string {
	dataDir := filepath.Join(sourceDir, "data")
	filePath := filepath.Join(dataDir, filename)
	content, err := os.ReadFile(filePath)
	if err != nil {
	...
	return generatePieChartFromJSON(content, title)
}

func generatePieChartFromJSON(content []byte, title string) string {
	var data []ChartDataPoint
	if err := json.Unmarshal(content, &data); err != nil {
		return fmt.Sprintf("<div class='chart-error'>Error parsing JSON: %s</div>", err.Error())
	}
...
func generateLineChart(sourceDir, filename, title string) string {
	dataDir := filepath.Join(sourceDir, "data")
	filePath := filepath.Join(dataDir, filename)
	content, err := os.ReadFile(filePath)
	if err != nil {
	...
	return generateLineChartFromJSON(content, title)
}

func generateLineChartFromJSON(content []byte, title string) string {
	var data []ChartDataPoint
	if err := json.Unmarshal(content, &data); err != nil {
		return fmt.Sprintf("<div class='chart-error'>Error parsing JSON: %s</div>", err.Error())
	}
	...
	for i, d := range data {
		...
		html += fmt.Sprintf(`<circle cx="%.1f" cy="%.1f" r="4" fill="var(--chart-1, var(--primary-color, #3b82f6))" />`, x, y)
	...
	// Draw line
	html += fmt.Sprintf(`<polyline fill="none" stroke="var(--chart-1, var(--primary-color, #3b82f6))" stroke-width="3" points="%s" />`, strings.TrimSpace(points))
