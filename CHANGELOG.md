...
| Version | Description |
| :--- | :--- |
| **v4.15.0** | **Multi-Theme Gallery & Robust Compliance**: <br>- **Unified Theme Incubator**: Added the `make build-all` and `make serve-all` workflows to compile, build, and serve all 15 Tamarind themes simultaneously under a single beautiful `public-all` gallery directory, maintaining pristine binary separation.<br>- **Responsive Image Scaling**: Injected a global, mobile-first responsive scaling pipeline to baseline stylesheet `shared/core.css` ensuring large optimized images never overflow narrow layout containers across all themes.<br>- **HTML Tag Compliance**: Enhanced the `AnalyzeCSS` engine to parse bare HTML tag selectors like `img` and `figure`, officially adding them as mandatory specifications in `THEME_SPEC.md` and verifying them across all themes in Go unit tests.<br>- **Local Workflow Skill**: Added the `.agent/workflows/tamarind-publish-themes.md` workflow for automated theme gallery management. |
| **v4.14.0** | **Automated Compliance Verifier**: Implemented the automated theme compliance verifier system (`spec_parser.go`, `css_parser.go`, `report_generator.go`, `theme_verifier_test.go`) to automatically validate every theme's `style.css` against the variables and component selectors required by `THEME_SPEC.md`. |
| **v4.13.0** | **Optional Theme Flag**: Enhanced the `quickstart` CLI command to accept an optional theme flag to bootstrap new sites using a custom base style. |
| **v4.12.1** | **Contact Context**: Updated personal context and details on the contact page templates. |
| **v4.12.0** | **Viral Distribution & Hook Polish**: Refined the pre-push and git hook hardening logic; finalized viral badge placement on the contact and landing templates. |
| **v4.11.4** | **Theme Specification Contract**: Formally authored `THEME_SPEC.md` and `style-guide.md` to define variable tokens and component classes that all compliant themes must implement. |
